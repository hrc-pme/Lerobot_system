                    ...
                    try:
                        if map_info['type'] == 'camera':
                            ...
                            if val is not None:
                                ...
                                pos = val[:3]
                                rot = val[3:7] # [qx, qy, qz, qw]
                                grip = val[7:]

                                if arm == 'left':
                                    if prev_rot_left is not None:
                                        # Check direction vs previous
                                        if np.dot(rot, prev_rot_left) < 0:
                                            rot = -rot # Flip
                                    prev_rot_left = rot
                                elif arm == 'right':
                                    if prev_rot_right is not None:
                                        if np.dot(rot, prev_rot_right) < 0:
                                            rot = -rot
                                    prev_rot_right = rot
                                
                                # Re-assemble
                                val = np.concatenate([pos, rot, grip]).astype(np.float32)
