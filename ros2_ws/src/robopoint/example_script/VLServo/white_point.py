...
from . import d415_helpers_without_pyrealsense as dh
...
def main(use_remote_computer: bool, x: int, y: int, push_radius_m: float,
    ...
    # Subscriber to D415 image stream
    d415_context = zmq.Context()
    d415_socket = d415_context.socket(zmq.SUB)
    d415_socket.setsockopt(zmq.SUBSCRIBE, b'')
    d415_socket.setsockopt(zmq.SNDHWM, 1)
    d415_socket.setsockopt(zmq.RCVHWM, 1)
    d415_socket.setsockopt(zmq.CONFLATE, 1)
    if use_remote_computer:
        d415_address = 'tcp://' + yn.robot_ip + ':' + str(yn.d415_port)
    else:
        d415_address = 'tcp://' + '127.0.0.1' + ':' + str(yn.d415_port)
    d415_socket.connect(d415_address)
    ...
    try:
        while True:
            ...
            d415_output = d415_socket.recv_pyobj()
            color_image = d415_output.get('color_image')
            depth_image = d415_output.get('depth_image')
            depth_camera_info = d415_output.get('depth_camera_info')
            depth_scale = d415_output.get('depth_scale')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--remote', action='store_true', help='Run on remote computer (bind publisher, subscribe to robot D415).')
    parser.add_argument('-x', type=int, required=True, help='Pixel x (columns) in the D415 color/depth image space.')
    parser.add_argument('-y', type=int, required=True, help='Pixel y (rows) in the D415 color/depth image space.')
