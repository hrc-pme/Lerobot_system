...
import numpy as np
import cv2
from .d415_helpers_without_pyrealsense import *
...
def start_d415(exposure): 
    ...
    for info in camera_info:
        ...
        if 'd415' in name_lower:
            d415_info = info
    if d415_info is None:
        print('D415 camera not found')
        print('Exiting')
        exit()
    else:
        print('D415 found:')
    ...
    config = rs.config()
    config.enable_device(d415_info['serial_number'])
