...
import time
import numpy as np
from . import d415_helpers as dh
...
def main(use_remote_computer, exposure):
    try: 
        ...
        pipeline, profile = dh.start_d415(exposure)
        first_frame = True
        ...
        if use_remote_computer:
            address = 'tcp://*:' + str(yn.d415_port)
        else:
            address = 'tcp://' + '127.0.0.1' + ':' + str(yn.d415_port)
        ...
        while True:
            ...
            if first_frame:
                ...
                d415_output = {'depth_camera_info' : depth_camera_info,
            ...
            d415_output['color_image'] = color_image
            d415_output['depth_image'] = depth_image
                
            socket.send_pyobj(d415_output)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        prog='Send D415 Images',
        description='Send D415 images to local and remote processes.'
    )
    parser.add_argument('-r', '--remote', action='store_true', help='Use this argument when allowing a remote computer to receive D415 images. Prior to using this option, configure the network with the file forcesight_networking.py on the robot and the remote computer.')
    parser.add_argument('-e', '--exposure', action='store', type=str, default='low', help=f'Set the D415 exposure to {dh.exposure_keywords} or an integer in the range {dh.exposure_range}') 
